...
<body>
  ...
  <div class="page_info">
    <?php
      $sql = "SELECT COUNT(id) as 'count' FROM simon198_blog_articles WHERE is_deleted = 0";
      $stmt = $conn->prepare($sql);
      $result = $stmt->execute();
      $result = $stmt->get_result();
      $row = $result->fetch_assoc();
      $total_page = ceil($row["count"] / $article_per_page);
    ?>
    總共有 <?php echo $row["count"]; ?> 篇文章，共 <?php echo $total_page; ?> 頁
    <div class='paginator'>
      <?php if ($page !== 1) {?>
      <a href='article_list.php?page=1>'>第一頁</a>
      ...
      <a href='article_list.php?page=<?php echo $total_page?>'>最末夜</a>
      <?php } ?>
    </div>
  </div>
  <div class='footer'>
